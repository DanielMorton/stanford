...
def create_chain_csp(n: int) -> CSP:
    ...
    for var in variables:
        csp.add_variable(var, [0, 1])  # Each variable can be either 0 or 1
    for i in range(1, n):
        csp.add_binary_factor(f'x{i}', f'x{i+1}', lambda x, y: x != y)  # Adjacent variables must be different
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n: int = 8) -> CSP:
    ...
    # Add variables
    for i in range(n):
        csp.add_variable(f'Q{i}', list(range(n)))  # Each queen can be in any column (0 to n-1)

    # Add binary factors
    for i in range(n):
        for j in range(i + 1, n):
            csp.add_binary_factor(f'Q{i}', f'Q{j}',
                lambda x, y, i=i, j=j: x != y and abs(x - y) != j - i)
...
class BacktrackingSearch:
    ...
    def get_unassigned_variable(self, assignment: Dict):
        if not self.mcv:
            ...
            for var in self.csp.variables:
                if var not in assignment:
                    # Count the number of values in the domain that satisfy all constraints
                    valid_values = sum(1 for val in self.domains[var]
                                       if self.satisfies_constraints(assignment, var, val))

                    # Update mcv if this variable has fewer valid values
                    if valid_values < min_domain_size:
                        min_domain_size = valid_values
                        mcv = var
                    elif valid_values == min_domain_size and self.csp.variables.index(var) < self.csp.variables.index(
                            mcv):
                        # Break ties by selecting the variable with the lowest index
                        mcv = var
...
class SchedulingCSPConstructor:
    ...
    def add_quarter_constraints(self, csp: CSP) -> None:
        ...
        # BEGIN_YOUR_CODE
        for request in self.profile.requests:
            if request.quarters:  # Only add constraints if quarters are specified
                for quarter in self.profile.quarters:
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                for cid in request.cids:
                    ...
                    # Add variable for course units
                    csp.add_variable((cid, quarter), list(range(course.minUnits, course.maxUnits + 1)) + [0])

                    # Constraint: if course is taken, units should be within range
                    csp.add_binary_factor((request, quarter), (cid, quarter),
                                          lambda req_cid, units: (req_cid != cid and units == 0) or
                                                                 (
                                                                             req_cid == cid and course.minUnits <= units <= course.maxUnits))

                    quarter_courses.append((cid, quarter))

            # Create sum variable for total units in the quarter
            total_units = create_sum_variable(csp, f'total_units_{quarter}', quarter_courses, self.profile.maxUnits)

            # Constraint: total units should be within allowed range
            csp.add_unary_factor(total_units, lambda units: self.profile.minUnits <= units <= self.profile.maxUnits)
        # END_YOUR_CODE
